...
#define  FALSE 0
u8 g_nSendNum = 3;
u8 g_bIap_Start = TRUE;
void Jump_Iap_Buf(void);
/*******************************************************************************
  * @函数名称	Receive_Packet
  * @函数说明   从发送端接收一个数据包
  * @输入参数   data ：数据指针
...
static int32_t Receive_Packet (uint8_t *data, int32_t *length, uint32_t timeout)
{
    ...
    *length = 0;
	...
	//开始接收一个包数据
    *data = c;
    for (i = 1; i < (packet_size + PACKET_OVERHEAD); i ++)   //接收包长度＝包头(3)+数据包+CRC(2)
    {
    ...
    if (data[PACKET_SEQNO_INDEX] != ((data[PACKET_SEQNO_COMP_INDEX] ^ 0xff) & 0xff))  //求反码
    {
        return -1;
    }
    *length = packet_size;
        ...
        for (packets_received = 0, file_done = 0, buf_ptr = buf; ;)
        {
            ...
            case 0:
                ...
                case 0:                          //接收到04 包长度为0
                    ...
                    //正常的数据包
                default:
                    if ((packet_data[PACKET_SEQNO_INDEX] & 0xff) != (packets_received & 0xff))   //包序号
                    {
                        Send_Byte(NAK);
                    }
                    else
                    {
                        if (packets_received == 0)  //第一个数据包
                        {
                            //文件名数据包
