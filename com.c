...
{
	//RS485_RX();
    if ( USART_GetFlagStatus(RS485_PORT, USART_FLAG_RXNE) != RESET)
    {
        *key = (uint8_t)RS485_PORT->DR;
		//RS485_TX();
        return 1;
    }
    else
...
{
    USART_SendData(RS485_PORT, c);
    while (USART_GetFlagStatus(RS485_PORT, USART_FLAG_TXE) == RESET)
    {
    }
...
{
	Delay(500);
	RS485_IAR_RX_TX_CONTROL_PORT->BSRR = RS485_IAR_RX_TX_PIN;
	Delay(1000);
...
{
	Delay(1500);
	RS485_IAR_RX_TX_CONTROL_PORT->BRR  = RS485_IAR_RX_TX_PIN;
	Delay(200);
}
